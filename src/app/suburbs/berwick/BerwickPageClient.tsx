...
export default function BerwickSuburbGuide({ initialProperties = [] }: { initialProperties?: any[] }) {
  const [activeSection, setActiveSection] = useState(0);
  const [isMobile, setIsMobile] = useState(false);
  const [isTablet, setIsTablet] = useState(false);
  const { properties: fetchedProperties } = useProperties({ suburb: 'Berwick', limit: 6 });
  // Server-fetched listings render in the initial HTML; client fetch takes over once it resolves
  const properties = fetchedProperties.length > 0 ? fetchedProperties : initialProperties;
          ...
          <section id="section-4" style={{
            ...
            <div style={{
              ...
              <h3 style={{
                ...
                Find Out What Your Berwick Home is Worth
              </h3>
              <p style={{
                ...
                Get a free, no-obligation appraisal from a local Berwick agent
              </p>
              <Link
                href="/appraisal"
                style={{
