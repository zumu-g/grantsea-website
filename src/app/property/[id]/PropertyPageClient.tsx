...
export default function PropertyPageClient() {
  ...
  const handleEnquiry = async (e: React.FormEvent) => {
    ...
    try {
      ...
      trackLead('property-enquiry');
      setEnquirySent(true);
      setTimeout(() => setEnquirySent(false), 3000);
    } catch (error) {
      console.error('Property enquiry failed:', error);
      setEnquiryError('Something went wrong sending your enquiry. Please call the agent instead.');
    }
  };
                ...
                <div>
                  {(() => {
                    ...
                    <div style={{
                      ...
                      {/* Show placeholder or property details for non-auction properties */}
                      {property.listingType === 'lease' ? 'Available for lease' : 'Private sale'}
                    </div>
                  )}
                </div>
